...
graham_cfg = []; 
graham_cfg.req_cores = 32; % maximum CPU cores per GPU request: 6 on Cedar, 16 on Graham.
graham_cfg.req_RAM = 64; % memory per node in GB
graham_cfg.req_queue='threaded';
graham_cfg.req_runtime='0-02:00'; % time (DD-HH:MM)
...
file_data_loop_variable = arrayfun(@(x)strsplit(file_data{x},{'for','='}),file_data_parfor_idx,'un',0); file_data_loop_variable = cellfun(@(x)x{2},file_data_loop_variable,'un',0);
...
for i = 1:size(start_idx,2)
    ...
    for m = 1:size(start_idx,1)
        file_data{file_data_parfor_idx(m)} = [for_text file_data_loop_variable{m} ' = ' num2str(curr_start_idx(m)) ':' num2str(curr_end_idx(m))];
    end
    ...
    function_name = [input_filename '_loopIDX_' regexprep(num2str(curr_start_idx),' ','_') 'to' regexprep(num2str(curr_end_idx),' ','_')];
...
loopIdx_string = strsplit(mat2str(1:size(start_idx,2)),{'[',']'}); loopIdx_string = loopIdx_string{2};
...
for m = 1:index_var_offset
    startIdx_string = [startIdx_string '${loopIDX_start' num2str(m) '[Idx]}']; if m~=index_var_offset startIdx_string = [startIdx_string '__'];end
    endIdx_string = [endIdx_string '${loopIDX_end' num2str(m) '[Idx]}']; if m~=index_var_offset endIdx_string = [endIdx_string '__'];end
end
shAll_file_data{10+(2*index_var_offset)+3} = startIdx_string;
shAll_file_data{10+(2*index_var_offset)+4} = endIdx_string;
shAll_file_data{10+(2*index_var_offset)+5} = ['curr_file=' input_filename '_loopIDX_${startIDX}to${endIDX}'];
