...
feature_names = {'COH', 'PAC', 'dPLI', 'CFC_SI'}; %  {'COH', 'PAC', 'PLI', 'dPLI', 'CFC_SI'}; potentially remove dPLI?
...
CONN_cfg.CONN_project_name = 'HCWMIVO'; % The name of the CONN project
...
CONN_cfg.window_step = 3; % in seconds - Used for Feature windows:
CONN_cfg.window_length = 3; % in seconds - Used for Feature windows:
...
offline_preprocess_cfg.ChannelCriterion = 0.8; % To reject channels based on similarity to other channels, cross-corr?
...
window_step = 1.5; % in seconds
window_length = 3; % in seconds, to increase if too computationally-intensive

% Set Paths:
base_path = fileparts(matlab.desktop.editor.getActiveFilename); cd(base_path); cd ..;
base_path = pwd;

% Include path:
toolboxes_path = [base_path filesep 'Toolboxes']; %base_path_main = [base_path filesep study_name filesep 'Main'];
...
base_path_data = [base_path_rd filesep study_name];
offline_preprocess_cfg.temp_file = [base_path_data filesep 'temp_deploy_files'];
...
% CONN_cfg.CONNresults_folder = [base_path filesep 'Models' filesep 'ICA_Labels' filesep CONN_cfg.CONN_project_name '-' CONN_cfg.CONN_analysis_name]; %need ICA labels###
% warning off
        ...
        try
            ...
            switch runs_to_include{jj}
                case 'task'
                    ...
                    task_dir = [curr_dir filesep 'EEG' filesep 'MInjury'];
                    fileName_dir = dir([task_dir filesep '*.cdt']);
                    % skip_analysis = isempty(fileName_dir) | isequal(dataset_name, '1001') | isequal(dataset_name, '1083') | isequal(dataset_name, '1085') | isequal(dataset_name, '1086') | isequal(dataset_name, '1005HCW') | isequal(dataset_name, '1084') | isequal(dataset_name, '1006HCW') | isequal(dataset_name, '1040HCW') | isequal(dataset_name, '1037HCW') | isequal(dataset_name, '1087') | isequal(dataset_name, '1089') | isequal(dataset_name, '1088') | isequal(dataset_name, '1090') | isequal(dataset_name, '1050HCW') | isequal(dataset_name, '1091') | isequal(dataset_name, '1092') | isequal(dataset_name, '1094') | isequal(dataset_name, '1096') | isequal(dataset_name, '1095') | isequal(dataset_name, '1098') | isequal(dataset_name, '1097') | isequal(dataset_name, '1099') | isequal(dataset_name, '1100') | isequal(dataset_name, '1104') | isequal(dataset_name, '1102') | isequal(dataset_name, '1006PSP') | isequal(dataset_name, '1101') | isequal(dataset_name, '1106') | isequal(dataset_name, '1110') | isequal(dataset_name, '1111') | isequal(dataset_name, '1112') | isequal(dataset_name, '1114') | isequal(dataset_name, '1103');%dmer1                   
                    skip_analysis = isempty(fileName_dir);% | ~isequal(dataset_name, '1001');
                    if ~skip_analysis% && ~(isfolder([curr_filepath filesep 'EEGfMRI_Raw']) && ~isempty([curr_filepath filesep 'EEGfMRI_Raw']))
                        curr_file = [fileName_dir.folder filesep fileName_dir.name];
                        [curr_filepath, curr_filename] = fileparts(curr_file);
                        [EEG] = loadcurry(curr_file);
                        
                        
                        if isempty(dir([curr_filepath filesep 'EEGfMRI_Raw']))
                            if (sum(isnan(EEG.data(:))) > 0)
                                EEGdata = EEG.data;
                                EEGdataLen = size(EEGdata,2);
                                chansToReject = zeros(1,EEG.nbchan);
                                parfor y = 1:EEG.nbchan
                                    subset = EEGdata(y,:);
                                    % subset = EEG.data(y,1:500);
                                    
                                    % Remove channels if too many NaNs:
                                    if sum(isnan(subset)) > 0.7*EEGdataLen
                                        chansToReject(y) = 1;
                                        ...
                                        if ~isempty(subset_nan_idx)
                                            for k = 1:length(subset_nan_idx)
                                                ...
                                                if isequal(subset_nan_idx(k),EEGdataLen) || isequal(subset_nan_idx(k),1)
                                                    subset(subset_nan_idx(k)) = 0.0;
                                                    %subset = EEGdata(y,:);
                                                    
%                                                 elseif subset_nan_idx(k) + interp_range > EEGdataLen || subset_nan_idx(k) - interp_range < 1
%                                                     
%                                                     upperLim = min(subset_nan_idx(k) + interp_range,EEGdataLen);
%                                                     lowerLim = max(subset_nan_idx(k) - interp_range,1);
%                                                     interp_range = 3;
%                                                     if subset_nan_idx(k) + interp_range > EEGdataLen || subset_nan_idx(k) - interp_range < 1
%                                                         subset(subset_nan_idx(k)) = 0.0;
%                                                         %subset = EEGdata(y,:);
%                                                     end
                                                    ...
                                                    upperLim = min(subset_nan_idx(k) + interp_range,EEGdataLen);
                                                    lowerLim = max(subset_nan_idx(k) - interp_range,1);
                                                    interp_x = [lowerLim : upperLim];
                                                    currSampIDX = find(interp_x == subset_nan_idx(k));
                                                    interp_x = [interp_x(1:currSampIDX-1) interp_x(currSampIDX+1:end)];
                                                    interp_y = subset(interp_x);
                                                    ...
                                                    if sum(isnan(interp_y)) > 0
                                                        ...
                                                        interp_x(replace_interp) = [];
                                                        interp_y(replace_interp) = [];
                                                    end
                                                    subset(subset_nan_idx(k)) = interp1(interp_x,interp_y,subset_nan_idx(k),'linear');
                                                end 
                                                
                                                disp(['Chan ' num2str(y) ' ' num2str(sum(isnan(subset))) ' remaining NaNs']);
...
%                                                 disp(subset_nan_idx(k));
                                ...
                                EEG.data = EEGdata;
                                if ~isempty(find(chansToReject))
                                    [EEG] = pop_select(EEG,'nochannel',find(chansToReject)); EEG = eeg_checkset(EEG);
                                end
                            end

                            EEG.setname = [runs_to_include{jj} '_' dataset_name '_MInjury']; EEG = eeg_checkset(EEG);
...
%                     end % GRAHAM_PARFOR_END
               case 'rest'
                    % curr_file = [curr_dir filesep 'EEG' filesep 'Inside_Scanner' filesep 'Acquisition 03.cdt'];                    
                    rest_dir = [curr_dir filesep 'EEG' filesep 'rsEEG_Pre'];
                    fileName_dir = dir([rest_dir filesep '*.cdt']);
                    % curr_file = [rest_dir filesep dataset_name '_resting_dataset.cdt']; skip_analysis = isempty(dir(curr_file));
                    skip_analysis = isempty(fileName_dir);
                    if ~skip_analysis 
                        curr_file = [fileName_dir.folder filesep fileName_dir.name];
                        [curr_filepath, curr_filename] = fileparts(curr_file);
                        [EEG] = loadcurry(curr_file);
                        ...
                        % EEG = pop_chanedit(EEG,'load',{chanlocs_file 'filetype' 'autodetect'}); - Add later
            ...
            curr_dir = [base_path_data filesep dataset_to_use];
...
% 
%                             task_dir = [curr_dir filesep 'Task_block_'
%                             num2str(m)];)
...
%                         end
                ...
                %% Remove unwanted EEG data outside of the relevant markers, start & end: %%Move this inside run_rest_EEGfMRI_preprocess
                % marker_instances = cellfun(@(x) strcmp(x,num2str(scan_parameters.slice_marker)),{EEG.event.type});
                % marker_instances_idx = find(marker_instances);
                % first_marker_start_time = EEG.event(marker_instances_idx(1)).latency/EEG.srate;
                % first_marker_end_time = EEG.event(marker_instances_idx(2)).latency/EEG.srate;
                % event_time = first_marker_end_time - first_marker_start_time;
                % last_marker_start_time = EEG.event(marker_instances_idx(length(marker_instances_idx))).latency/EEG.srate;
                % last_marker_end_time = last_marker_start_time + event_time;
                % % EEG = pop_rmdat(EEG, {num2str(scan_parameters.slice_marker)},[0 event_time],0);
                % EEG = pop_select(EEG, 'time', [first_marker_start_time last_marker_end_time]);
                % EEG = pop_saveset(EEG, 'filename',['rest_' dataset_name '_preprocessed.set'],'filepath',[task_dir filesep 'PreProcessed']);
                ...
                if scan_parameters.slice_marker == -1
                    ...
                    if length(size(EEG.data)) == 2
                        ...
                        end_idx = min((start_idx + (CONN_cfg.window_length*EEG.srate)),size(EEG.data,2)) - 1;
                    ...
                    curr_dataset_name = [runs_to_include{jj} '_' dataset_name '_MInjury'];
                    task_dir = [curr_dir];
...
%                         Featurefiles_basename = ['Rev_' curr_dataset_name];
%                         
%                         if ~isempty(dir([Featurefiles_directory filesep Featurefiles_basename '_ClassificationResults' name_suffix '_FEAT' final_featuresToUse '_CLASS' CONN_cfg.class_types '.mat'])) && ~redo_classification
%                             fprintf(['\n ***************************** Classification already done - Loading results for run ' num2str(m) ' ***************************** \n']);
%                             temp = load([Featurefiles_directory filesep Featurefiles_basename '_ClassificationResults' name_suffix '_FEAT' final_featuresToUse '_CLASS' CONN_cfg.class_types]);
%                             
...
%                                 case 'individual'
%                                     % if isempty(dir([task_dir filesep 'EEG_Features' filesep '*_CLASS' CONN_cfg.class_types '_mRMRiterateResults.mat']))
%                                         % missing_curate_featues_mRMR = 1; % REMOVE THIS LATER
%                                         % curate_features_mRMR_compiled(Featurefiles_basename, Featurefiles_directory, YY_final, max_features, task_dir, base_path)
%                                         curate_features_mRMR_compiled([Featurefiles_basename '_CLASS' CONN_cfg.class_types 'NEW'], Featurefiles_directory, YY_final, max_features, task_dir, base_path)
%                                     % end
%                                     if ~missing_curate_features_mRMR % REMOVE THIS LATER
%                                         mRMRiterateResults = load([Featurefiles_directory filesep Featurefiles_basename '_CLASS' CONN_cfg.class_types 'NEW' '_mRMRiterateResults.mat']);
...
%                                     if isempty(dir([task_dir filesep 'EEG_Features' filesep '*_CLASS' CONN_cfg.class_types '_FeatureSelectionResults.mat']))
%                                         [Features_ranked_mRMR, Features_scores_mRMR] = mRMR(Features,YY_final',max_features);
%                                         save([Featurefiles_directory filesep Featurefiles_basename '_CLASS' CONN_cfg.class_types '_FeatureSelectionResults'],'Features','Features_ranked_mRMR','Features_scores_mRMR');
%                                     else
%                                         load([Featurefiles_directory filesep Featurefiles_basename '_CLASS' CONN_cfg.class_types '_FeatureSelectionResults.mat']);
%                                     end
%                                     % [Features_ranked_mRMR, Features_scores_mRMR] = mRMR(Features,YY_final',max_features);
...
%                                 save([Featurefiles_directory filesep Featurefiles_basename '_ClassificationResults' name_suffix '_FEAT' final_featuresToUse '_CLASS' CONN_cfg.class_types '_NEW'],'TrainAccuracy','TestAccuracy','Model','TrainAccuracy_null','TestAccuracy_null','Model_null','testIdx','trainIdx','trial_select_bin','class_weights');
%                                 
...
%                     save([Featurefiles_directory filesep Featurefiles_basename '_FeatureSelectionResults_SUB' '_FEAT' final_featuresToUse '_CLASS' CONN_cfg.class_types],'Features_SUB','Features_ranked_mRMR_SUB','Feature_labels_mRMR_SUB','Features_scores_mRMR_SUB');
%                 else
...
%                 
%                 % Run Classification:
%                 if ~isempty(dir([curr_dir filesep Featurefiles_basename '_ClassificationResults' name_suffix '_SUB' '_FEAT' final_featuresToUse '_CLASS' CONN_cfg.class_types '.mat']))
%                     load([curr_dir filesep Featurefiles_basename '_ClassificationResults' name_suffix '_SUB' '_FEAT' final_featuresToUse '_CLASS' CONN_cfg.class_types]);
%                 else
...
% if save_finalData
%    finalData_name = [base_path_main filesep 'FinalResults_' study_name '_FEAT' final_featuresToUse '_CLASS' CONN_cfg.class_types]; 
