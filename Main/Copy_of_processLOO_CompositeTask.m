...
for current_test_block = 2:length(sub_dir)
    ...
    testIdx_SUB = 1:size(curr_Features_test,1); trainIdx_SUB = testIdx_SUB(end)+[1:size(curr_Features_train,1)];
    ...
    for kk = 1:num_CV_folds_per_pt % make this par
        ...
        [TrainAccuracy_null_SUB(current_test_block,kk), TestAccuracy_null_SUB(current_test_block,kk), Model_null_SUB{current_test_block,kk}] = classify_SVMweighted_libsvm(curr_Features_classify(:,Features_ranked_mRMR_SUB),Y_null','RBF',0,class_weights,trainIdx_SUB{kk},testIdx_SUB{kk});
    end
