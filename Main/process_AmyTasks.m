        ...
        try
            ...
            switch runs_to_include{jj}
                case 'task'
                    ...
                    EEG_vhdr = cell(size(curr_tD_file.block_onset_vect)); saved_final_data = cell(size(curr_tD_file.block_onset_vect));
...
%                 end % GRAHAM_PARFOR_END
                ...
                redo_classification = 0; % If '1', do the classification again, otherwise load results from dataset saved
                ...
                for m = 1:length(EEG_vhdr) % GRAHAM_PARFOR-2
                    ...
                    if ~isnan(EEGfMRI_corrIDX(ii,m))
                        ...
                        YY_final = cell2mat(CONN_data.fMRI_labels_selected_window_avg_thresh{ii}{curr_CONN_IDX});  % NOTE:only because this is excluding the first subject
                        YY_final_continuous = (CONN_data.fMRI_labels_selected_window_avg{ii}{curr_CONN_IDX}); YY_final_continuous = cat(1,YY_final_continuous{:}); % NOTE:only because this is excluding the first subject
                        ...
                        if ~isempty(dir([Featurefiles_directory filesep Featurefiles_basename '_ClassificationResults' name_suffix '_FEAT' final_featuresToUse '_CLASS' CONN_cfg.class_types '_NEW' '.mat'])) && ~redo_classification
                            ...
                            % Verify the length of the labels and features
                            % are the same:
                            currFeatures_dir = dir([task_dir filesep 'EEG_Features' filesep 'Rev_' curr_dataset_name '_Epoch*.mat']);
                            currFeatures_finished = cellfun(@(x) strsplit(x,{'Epoch','.mat'}),{currFeatures_dir.name},'un',0); currFeatures_finished = cellfun(@(x) str2num(x{2}),currFeatures_finished);
                            if length(YY_final) > max(currFeatures_finished)
                                YY_final = YY_final(1:max(currFeatures_finished));
                                YY_final_continuous = YY_final_continuous(1:max(currFeatures_finished),:);
                                YY_final_continuous_thresh = YY_final_continuous_thresh(1:max(currFeatures_finished),:);
                            end
                            ...
                            if ~missing_curate_features_mRMR % REMOVE THIS LATER
                                ...
                                % save([Featurefiles_directory filesep Featurefiles_basename '_ClassificationResults' name_suffix '_FEAT' final_featuresToUse '_CLASS' CONN_cfg.class_types],'TrainAccuracy','TestAccuracy','Model','TrainAccuracy_null','TestAccuracy_null','Model_null','testIdx','trainIdx');
