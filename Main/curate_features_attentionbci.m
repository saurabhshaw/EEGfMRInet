function [compute_feat, varargout] = curate_features_attentionbci(feature_names, featureVar_to_load, Featurefiles_basename, Featurefiles_directory, cell_output, provide_final)
...
if ~isempty(Featurefiles_dir) && isempty(Featurefiles_curated_dir)
    ...
    parfor i = 1:length(Features_to_process)
                    ...
                    for n = 1:size(curr_Feature{1}{1},2) % Number of Frequency Windows
                        ...
                        if size(curr_Feature{1}{m},1) == 1 % Single Frequency Computation
                            ...
                            for p = 1:size(curr_Feature{1}{1},1)
                                if ~isempty(curr_Feature{1}{m}{p,n}) curr_Feature_curated(m,p,n,:,:) = curr_Feature{1}{m}{p,n}; end     
                            end
                        end
                    end
                end
        ...
        parsave([Featurefiles_directory filesep Featurefiles_basename '_AllEpochs_' Features_to_process{i}],Feature);
    end
else
