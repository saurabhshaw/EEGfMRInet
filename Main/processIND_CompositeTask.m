...
%% Run Loop:
for ii = 8:length(sub_dir) % GRAHAM_PARFOR-1
...
%                 
                ...
                for m = 1:length(EEG_vhdr) % GRAHAM_PARFOR-2
                    ...
                    if ~isnan(EEGfMRI_corrIDX(ii,m))
                        ...
                        for tt = 1:size(YY_final_continuous_thresh,2)
                            
                            % Set the corresponding output labels:
                            YY_final = YY_final_continuous_thresh(:,tt)';
                            ...
                            switch final_featuresToUse
                                case 'individual'                                    
                                    if isempty(dir([task_dir filesep 'EEG_Features' filesep '*_mRMRiterateResults_individual' num2str(tt) '.mat']))
                                        curate_features_mRMR_individual_compiled(Featurefiles_basename, Featurefiles_directory, YY_final, max_features, task_dir, base_path, num2str(tt))
                                    end
                                ...
                                case 'preselected'                                    
                                    curr_model = load(model_file{tt}); % Rewrite this to include multiple model_files
                            ...
                            if final_featureSelection
                                if isempty(dir([task_dir filesep 'EEG_Features' filesep '*_FeatureSelectionResults_individual' num2str(tt) '.mat']))
                                    [Features_ranked_mRMR, Features_scores_mRMR] = mRMR(Features,YY_final',max_features);
                                    save([Featurefiles_directory filesep Featurefiles_basename '_FeatureSelectionResults_individual' num2str(tt)],'Features','Features_ranked_mRMR','Features_scores_mRMR');
                                else
                                    load([Featurefiles_directory filesep Featurefiles_basename '_FeatureSelectionResults_individual' num2str(tt) '.mat']);
                                end
                                % [Features_ranked_mRMR, Features_scores_mRMR] = mRMR(Features,YY_final',max_features);
                            ...
                            %if ~individual_network_classif
                                ...
                                parfor kk = 1:num_CV_folds % Make this par
                                    ...
                                    [testIdx{kk}, trainIdx{kk}] = testTrainIdx_overlap(size(Features,1),CONN_cfg.window_length/scan_parameters.TR,1-testTrainSplit);
                                    % [TrainAccuracy(m,kk), TestAccuracy(m,kk), Model{kk}] = classify_SVM_libsvm(Features(:,Features_ranked_mRMR),YY_final','RBF',0,trainIdx{kk},testIdx{kk});
                                    [TrainAccuracy(m,kk,tt), TestAccuracy(m,kk,tt), Model{kk,tt}] = classify_SVMweighted_libsvm(Features,YY_final','RBF',0,class_weights,trainIdx{kk},testIdx{kk});
...
%                             end
                            ...
                            save([Featurefiles_directory filesep Featurefiles_basename '_ClassificationResults_individual'],'TrainAccuracy','TestAccuracy','Model','TrainAccuracy_null','TestAccuracy_null','Model_null','testIdx','trainIdx');
