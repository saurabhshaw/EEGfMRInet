...
if isempty(dir([curr_dir filesep 'EEG_Features_Dan']))
    mkdir([curr_dir filesep 'EEG_Features_Dan']);
end
    ...
    switch curr_feature   
        ...
        case 'FBCSP' % Compute FBCSP features:
            ...
            for i = 1:size(EEG.data,3)
                curr_EEG_cell{i} = squeeze(EEG.data(:,:,i));
            end
         ...
         case 'BPow' % Compute Frequency Band powers:
            compute_BPow = 1;
            [bandPower_features] = EEG_band_powers(EEG.data, EEG.srate); 
...
for j = 1:num_epochs
    ...
    filename = [curr_dir filesep 'EEG_Features_Dan' filesep 'Rev_Sub' num2str(ii) '_Ses' num2str(jj) '_Epoch' num2str(j)];
    % filename = [curr_path filesep 'EEG_Results' filesep 'Rev_Sub' num2str(subject_number) '_Ses' num2str(session_number) '_Epoch' num2str(j)];
    parsave(filename, analyzedData)
end

%% Save all epochs accumulated in Features cell:
% final_filename = [curr_dir filesep 'EEG_Features' filesep 'Rev_Sub' num2str(ii) '_Ses' num2str(jj) '_AllEpochs'];
