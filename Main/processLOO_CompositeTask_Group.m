...
for kk = 1:num_CV_folds % make this par
    ...
    [TrainAccuracy_SUB(kk), TestAccuracy_SUB(kk), Model_SUB{kk}] = classify_SVMweighted_libsvm(curr_Features(:,Features_ranked_mRMR_SUB),curr_YY_final','RBF',0,class_weights,trainIdx_SUB{kk},testIdx_SUB{kk});
    
    % Run null classification (after randomly permuting the labels of the testing trials):
    Y_null_train = curr_YY_final(trainIdx_SUB{kk}); Y_null = curr_YY_final;
    ...
    [TrainAccuracy_null_SUB(kk), TestAccuracy_null_SUB(kk), Model_null_SUB{kk}] = classify_SVMweighted_libsvm(curr_Features(:,Features_ranked_mRMR_SUB),Y_null','RBF',0,class_weights,trainIdx_SUB{kk},testIdx_SUB{kk});
     
    final_saveName = [base_path_main filesep 'FinalResults_' study_name 'LOO' '_FEAT' final_featuresToUse '_CLASS' CONN_cfg.class_types '_CVrun' num2str(kk)];
