...
for kk = 1:num_CV_folds % make this par
    ...
    current_train_blocks{kk} = setdiff([1:length(sub_dir)],current_test_block{kk});
    ...
    select_testIDX = current_test_block{kk} == curr_subIDX; 
    select_trainIDX = zeros(size(curr_subIDX)); for i = 1:length(current_train_blocks{kk}) curr_bool = current_train_blocks{kk}(i) == curr_subIDX; select_trainIDX = select_trainIDX | curr_bool; end
    ...
    [TrainAccuracy_SUB(kk), TestAccuracy_SUB(kk), Model_SUB{kk}] = classify_SVMweighted_libsvm(curr_Features_classify(:,Features_ranked_mRMR_SUB),curr_YY_final_classify','RBF',0,class_weights,trainIdx_SUB{kk},testIdx_SUB{kk});
    
    % Run null classification (after randomly permuting the labels of the testing trials):
    Y_null_train = curr_YY_final_classify(trainIdx_SUB{kk}); Y_null = curr_YY_final_classify;
    Y_null(trainIdx_SUB{kk}) = Y_null_train(randperm(length(Y_null_train)));
    ...
    [TrainAccuracy_null_SUB(kk), TestAccuracy_null_SUB(kk), Model_null_SUB{kk}] = classify_SVMweighted_libsvm(curr_Features_classify(:,Features_ranked_mRMR_SUB),Y_null','RBF',0,class_weights,trainIdx_SUB{kk},testIdx_SUB{kk});
     
    final_saveName = [Results_outputDir filesep 'FinalResults_' study_name 'LOO' '_FEAT' final_featuresToUse '_CLASS' CONN_cfg.class_types 'NEW' '_CVrun' num2str(kk)];
    ...
    fprintf(['\n ***************************** Finished Subject ' sub_dir_mod(current_test_block{kk}).PID ' ***************************** \n']);               
end

% Save final data:
final_saveName = [Results_outputDir filesep 'FinalResults_' study_name 'LOO' '_FEAT' final_featuresToUse '_CLASS' CONN_cfg.class_types 'NEW'];
