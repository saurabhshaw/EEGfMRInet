...
for current_test_block = 2:length(sub_dir) % GRAHAM_PARFOR-1
    ...
    % For testing only:
    % current_train_blocks = current_train_blocks(unique(randi(length(current_train_blocks),[1 5])));
    current_train_blocks_rand = current_train_blocks(randperm(length(current_train_blocks)));
    current_train_blocks = current_train_blocks_rand(1:5);
    ...
    if isempty(dir([final_saveName '.mat']))
        ...
        [TrainAccuracy, TestAccuracy, C] = classify_EnsembleTrees_matlab(X,Y,numTrees,testTrainPartition,regression,varargin)
    ...
    % dataset
    name_suffix = ['_IND' sub_dir_mod(current_test_block).PID];
...
%     end
    ...
    for i = 1:length(currFeatures_curated)
        INDcurr_Features_struct = load([output_base_path_data filesep 'Analyzed_data' filesep 'GroupResults_' study_name filesep 'LeaveOneOut_IND_Features' filesep study_name '_' currFeatures_curated{i} '_mRMRiterateGroupResults_' CONN_cfg.class_types name_suffix]);
