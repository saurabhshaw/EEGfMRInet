...
for current_test_block = 2:length(sub_dir)
    ...
    %for kk = 1:num_CV_folds_per_pt % make this par
    ...
    if isempty(dir([final_saveName '.mat']))        
        ...
        % [TrainAccuracy_SUB(current_test_block,kk), TestAccuracy_SUB(current_test_block,kk), Model_SUB{current_test_block,kk}] = classify_SVMweighted_libsvm(curr_Features_classify(:,Features_ranked_mRMR_SUB),curr_YY_final_classify','RBF',0,class_weights,trainIdx_SUB{kk},testIdx_SUB{kk});
        ...
        % [TrainAccuracy_SUB(current_test_block), TestAccuracy_SUB(current_test_block), Model_SUB{current_test_block}] = classify_EnsembleTrees_matlab(curr_Features_classify(:,Features_ranked_mRMR_SUB),curr_YY_final_classify',100,0,0,trainIdx_SUB,testIdx_SUB);
        ...
        % [TrainAccuracy_null_SUB(current_test_block), TestAccuracy_null_SUB(current_test_block), Model_null_SUB{current_test_block}] = classify_EnsembleTrees_matlab(curr_Features_classify(:,Features_ranked_mRMR_SUB),Y_null',100,0,0,trainIdx_SUB,testIdx_SUB);
...
% Save final data:
final_saveName = [Results_outputDir filesep 'FinalResults_' variant '_' study_name 'LOO' '_FEAT' final_featuresToUse '_CLASS' CONN_cfg.class_types 'NEW' '_Feat' arrayfun(@(x) num2str(x),features_to_include)];
