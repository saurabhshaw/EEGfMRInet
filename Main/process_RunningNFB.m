...
%                 
                ...
                if task_end_block(end) > length(Yhat_final) task_end_block(end) = length(Yhat_final); end
                Yhat_task = arrayfun(@(x,y)Yhat_final(x:y),task_start_block,task_end_block,'un',0);
                Y_labels = sort(unique(Yhat_final));
                if length(Y_labels) < length(state_names) Y_labels = state_names; end % Added this later since it was missing the ones with the missing network label
