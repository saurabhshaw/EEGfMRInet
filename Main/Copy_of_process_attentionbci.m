...
dataset_name = 'frontSpeakersAlternating_events'; %For speaker_audio ; run1 = speaker alternating ; run2 = speaker simultaneous 
...
for ii = 1:length(sub_dir)
    for jj = 1:length(runs_to_include)        
        ...
        trial_data = cellfun(@(x) x{1},{EEG.epoch(:).eventtype},'UniformOutput',0); trial_data_unique = unique(trial_data); % Convert labels to numbers
