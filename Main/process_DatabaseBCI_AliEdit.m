...
modality = 'EEG';
dataset_name = 'P2001_Pretrain';

% Control Parameters:
runs_to_include = {'task'}; % Conditions - From the description of the dataset
seconds_to_cut = 20; srate = 512; % actual rate is: 2048
...
num_of_channels = 64+2; %was 130

% Setup offline_preprocess_cfg:
offline_preprocess_cfg = [];
offline_preprocess_cfg.filter_lp = 0.05; % Was 1 Hz
offline_preprocess_cfg.filter_hp = 100; % Was 40 Hz
...
offline_preprocess_cfg.temp_file = 'E:\School\temp_folder'
...
%[base_path_rc, base_path_rd] = setPaths();    was uncommented
%base_path_rd = ['F:\Saurabh_files\Research_data'];     %was commented
base_path_rd = ['E:\School\Pilot study'];     %was commented
base_path_data = base_path_rd;
output_base_path_data = base_path_data; %GRAHAM_OUTPUT_PATH
%I COMMENTED THIS: distcomp.feature( 'LocalUseMpiexec', false )

%% Process the participants' data:
% dataset_to_use = [file_date '_' study_name '_' modality '-' file_ID];
% dataset_name = file_ID;
chanlocs_file = [base_path filesep 'Cap_files' filesep 'biosemi64_updated.sfp'];
...
for ii = 1:length(sub_dir) 
    ...
    for jj = 1:length(runs_dir)
        try
            ...
            EEG = pop_select(EEG, 'nochannel', [1 num_of_channels:length(EEG.chanlocs)]);
            ...
            curr_type = 'Im';
            offline_preprocess_cfg.task_segment_end = 8; % end of the segments in relation to the marker
...
%% Curate results across all tasks:
total_numTasks = 44;
TrainAccuracy_Global = nan(total_numTasks,total_numTasks,num_CV_folds); 
TestAccuracy_Global = nan(total_numTasks,total_numTasks,num_CV_folds); 

for i = 1:length(Y_unique) % GRAHAM_PARFOR-1
    for j = 1:length(Y_unique) % GRAHAM_PARFOR-2
        if i < j
            TrainAccuracy_Global(Y_unique(i),Y_unique(j),:) = squeeze(TrainAccuracy(i,j,:));
            TestAccuracy_Global(Y_unique(i),Y_unique(j),:) = squeeze(TestAccuracy(i,j,:));
        end
    end
end

save([curr_dir_data filesep dataset_name '_GlobalClassificationResults'],'TrainAccuracy','TestAccuracy');
