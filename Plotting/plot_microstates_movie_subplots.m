...
function [Movie, Colormap] = plot_microstates_movie_subplots(data,srate,eloc_locs,varargin);
...
for f = 1:length(opt.movieframes)                      % make the movie, frame by frame
   ...
   if strcmpi(opt.mode, '2d')
       subplot(2,2,[1,2]);topoplot(data(:,indFrame),eloc_locs,'maplimits',opt.minmax, opt.topoplotopt{:}); 
   else
