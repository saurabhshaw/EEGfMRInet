function [arr_sortamp, arr_plotamp, arr_avgamp] = phase_amplitude_coupling_function_custom(dataset,higher_freq_idx,lower_freq_idx,PAC_opt)
...
try    
    ...
    arr_sortamp = zeros(numberOfBin, 2, segment_num);
    % arr_ch_sortamp = zeros(numberOfBin, 2, segment_num, length(channels_pac));
    arr_plotamp = zeros(numberOfBin,segment_num);
    arr_avgamp = zeros(1,segment_num);
    
    % Repeat the calculation below for each segment
    for segment = 1:segment_num
        ch_sortamp = zeros([numberOfBin, 2, size(channels_pac,1)]);
        for n = 1:size(channels_pac,1)    % Find average over all channels
            ...
            for i = (((segment-1)*segment_pnts)+1):sub
                for j = 1:numberOfBin 
                    if ch_phase(i) > (-pi+(j-1)*binsize) && ch_phase(i) < (-pi+(j*binsize))
                        ...
                        ch_sortamp(j,1,n) = ch_sortamp(j,1,n) + ch_amp(i);
                        ch_sortamp(j,2,n) = ch_sortamp(j,2,n) + 1;
                        break;
                    end
                end
            end
...
%         end
        ...
        arr_sortamp(:,:,segment) = squeeze(sortamp);
        arr_plotamp(:,segment) = squeeze(plotamp);
        arr_avgamp(segment) = squeeze(avgamp);
