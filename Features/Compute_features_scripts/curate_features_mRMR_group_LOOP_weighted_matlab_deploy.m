...
function curate_features_mRMR_group_LOOP_weighted_matlab_deploy(i,Featurefiles_directory_ALL,curr_Featurefiles_basename_ALL,YY_final_ALL,weights,name_suffix,output_base_path_data,study_name,max_features,CONN_cfg,varargin)
...
dir_checkpoints_IND = dir([output_base_path_data filesep 'Analyzed_data' filesep 'GroupResults_' study_name filesep 'mRMRiterate_' currFeatures_curated{i} '_' CONN_cfg.class_types '_CHECKPOINT_' '*' name_suffix '.mat']);
dir_checkpoints = dir_checkpoints_IND;
...
if ~isempty(dir_checkpoints)
    ...
    dir_checkpoints_num = cellfun(@(x)strsplit(x,{'_CHECKPOINT_',name_suffix,'.mat'}),{dir_checkpoints.name},'un',0);
...
if length(feat_file_Feature_size) == 4 % Single Frequency Computation
    ...
    parfor m = 1:feat_file_Feature_size(1) % Number of Windows = WAS PARFOR
        ...
        for n = 1:feat_file_Feature_size(2) % Number of Frequency Windows
            ...
            [elec_output_features{n}, elec_output_scores{n},elec_dataset_mRMR,elec_feature_labels_mRMR] = mRMR_weighted_matlab_iterate_loop(curr_dataset_mRMR,curr_labels_mRMR_final,weights,innermost_feature_size,max_features,elec_dataset_mRMR,elec_feature_labels_mRMR);
            % [elec_output_features{n}, elec_output_scores{n},elec_dataset_mRMR{n},elec_feature_labels_mRMR{n}] = mRMR_weighted_matlab_iterate_loop(curr_dataset_mRMR,curr_labels_mRMR,weights,curr_feature_size,max_features,elec_dataset_mRMR{n},elec_feature_labels_mRMR{n});
        ...
        % [freq_output_features{m}, freq_output_scores{m},freq_dataset_mRMR,freq_feature_labels_mRMR] = mRMR_weighted_matlab_iterate_loop(elec_dataset_mRMR,curr_labels_mRMR,curr_feature_size,max_features,freq_dataset_mRMR,freq_feature_labels_mRMR,elec_feature_labels_mRMR);
        [freq_output_features{m}, freq_output_scores{m},freq_dataset_mRMR{m},freq_feature_labels_mRMR{m}] = mRMR_weighted_matlab_iterate_loop(elec_dataset_mRMR,curr_labels_mRMR_final,weights,curr_feature_size,max_features,freq_dataset_mRMR{m},freq_feature_labels_mRMR{m},elec_feature_labels_mRMR);
    ...
    try % In case it is unreadable
        ...
        load([output_base_path_data filesep 'Analyzed_data' filesep 'GroupResults_' study_name filesep 'mRMRiterate_' currFeatures_curated{i} '_' CONN_cfg.class_types '_CHECKPOINT_' num2str(last_checkpoint) name_suffix]);
    catch
    ...
    %for m = 1:feat_file_Feature_size(1) % Number of Windows
        ...
        parfor n = 1:(feat_file_Feature_size(2)-1) % Number of Frequency Windows
            ...
            for p = 1:feat_file_Feature_size(3)
                ...
                if n < p % This is how features were computed for cross-frequency features
                    ...
                    % Run mRMR at this level:
                    [innerFreq_output_features{p}, innerFreq_output_scores{p},innerFreq_dataset_mRMR,innerFreq_feature_labels_mRMR] = mRMR_weighted_matlab_iterate_loop(curr_dataset_mRMR,curr_labels_mRMR_final,weights,innermost_feature_size,max_features,innerFreq_dataset_mRMR,innerFreq_feature_labels_mRMR);
                    % [innerFreq_output_features{p}, innerFreq_output_scores{p},innerFreq_dataset_mRMR{p},innerFreq_feature_labels_mRMR{p}] = mRMR_weighted_matlab_iterate_loop(curr_dataset_mRMR,curr_labels_mRMR,weights,curr_feature_size,max_features,innerFreq_dataset_mRMR{p},innerFreq_feature_labels_mRMR{p});
            ...
            [elec_output_features{n}, elec_output_scores{n},elec_dataset_mRMR{n},elec_feature_labels_mRMR{n}] = mRMR_weighted_matlab_iterate_loop(innerFreq_dataset_mRMR,curr_labels_mRMR_final,weights,curr_feature_size,max_features,elec_dataset_mRMR{n},elec_feature_labels_mRMR{n},innerFreq_feature_labels_mRMR,innerFreq_loopIDX_mRMR);
            % [elec_output_features{n}, elec_output_scores{n},elec_dataset_mRMR,elec_feature_labels_mRMR] = mRMR_weighted_matlab_iterate_loop(innerFreq_dataset_mRMR,curr_labels_mRMR,weights,curr_feature_size,max_features,elec_dataset_mRMR,elec_feature_labels_mRMR,innerFreq_feature_labels_mRMR);
        ...
        elec_feature_labels_mRMR = cat(2,elec_feature_labels_mRMR{:});
        curr_feature_size = [feat_file_Feature_size(2) size(elec_output_features{1},2)];
        [freq_output_features{m}, freq_output_scores{m},freq_dataset_mRMR,freq_feature_labels_mRMR] = mRMR_weighted_matlab_iterate_loop(elec_dataset_mRMR,curr_labels_mRMR_final,weights,curr_feature_size,max_features,freq_dataset_mRMR,freq_feature_labels_mRMR,elec_feature_labels_mRMR,elec_loopIDX_mRMR);
        % [freq_output_features{m}, freq_output_scores{m},freq_dataset_mRMR{m},freq_feature_labels_mRMR{m}] = mRMR_weighted_matlab_iterate_loop(elec_dataset_mRMR,curr_labels_mRMR,weights,curr_feature_size,max_features,freq_dataset_mRMR{m},freq_feature_labels_mRMR{m},elec_feature_labels_mRMR);
...
% [wind_output_features{i}, wind_output_scores{i},wind_dataset_mRMR,wind_feature_labels_mRMR] = mRMR_weighted_matlab_iterate_loop(freq_dataset_mRMR,curr_labels_mRMR,curr_feature_size,weights,max_features,freq_dataset_mRMR,wind_feature_labels_mRMR,freq_feature_labels_mRMR);
[wind_output_features{i}, wind_output_scores{i},wind_dataset_mRMR,wind_feature_labels_mRMR] = mRMR_weighted_matlab_iterate_loop(freq_dataset_mRMR,curr_labels_mRMR_final,weights,curr_feature_size,max_features,wind_dataset_mRMR,wind_feature_labels_mRMR,freq_feature_labels_mRMR);
