function curate_features_mRMR_individual_checkpoint_deploy(input_mat)
...
last_checkpoint = 0;
if isempty(dir([Featurefiles_directory filesep 'checkpoints' filesep '*.mat'])) % If there are no previous checkpoints
    ...
    last_checkpoint_data = load([Featurefiles_directory filesep 'checkpoints' filesep Featurefiles_basename '_mRMRiterateResults_individual' ind_name '_CHECKPOINT' num2str(last_checkpoint)]);
...
for i = last_checkpoint_data.i:length(currFeatures_curated) % Number of features
    ...
    if length(feat_file_Feature_size) == 4 % Single Frequency Computation
        ...
        parfor m = windows_to_process % Number of Windows
