function curate_features_mRMR_group_checkpoint_deploy_collectDist(input_mat)
...
%         end
    ...
    num_feat_timepnts = num_feat_timepnts; % For satisfying parfor requirements
...
%                 % curr_dataset_mRMR = cell2mat(cellfun(@(x)x(:),curr_dataset_mRMR,'un',0))';
        ...
        for m = dir_checkpoints_num_sorted
...
%             toc
            ...
            tic
            load([output_base_path_data filesep 'Analyzed_data' filesep 'GroupResults_' study_name filesep 'mRMRiterate_' currFeatures_curated{i} '_' CONN_cfg.class_types '_CHECKPOINT_' num2str(m)],'elec_dataset_mRMR','elec_feature_labels_mRMR','elec_output_features','elec_output_scores');
            
%             elec_dataset_mRMR = temp.elec_dataset_mRMR; elec_feature_labels_mRMR = temp.elec_feature_labels_mRMR;
%             elec_output_features = temp.elec_output_features; elec_output_scores = temp.elec_output_scores;
%                         
            % Run mRMR at this level:
            elec_loopIDX_mRMR = [];
            ...
            toc
            disp(['Finished mRMR loop for function ' currFeatures_curated{i} ' Window ' num2str(m)]);
        end
    end
...
save([output_base_path_data filesep 'Analyzed_data' filesep 'GroupResults_' study_name filesep study_name '_' currFeatures_curated{i} '_mRMRiterateGroupResults_' CONN_cfg.class_types],'final_dataset_mRMR','final_feature_labels_mRMR','curr_labels_mRMR','curr_dataset_mRMR_IDX','currFeatures_curated','nestedFeature_struct','-v7.3');
