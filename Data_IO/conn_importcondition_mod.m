function out=conn_importcondition_mod(filename,study_conditions,varargin)
...
names = study_conditions; nconds = cellfun(@(x)find(cellfun(@(y)~isempty(strmatch(x,y,'exact')),study_conditions)),conditions);
        ...
        for ncond=1:max(nconds),
            ...
            if ~isempty(data)
                ...
                if iscell(onsets)
                    ...
                    t1=cellfun(@str2num,onsets(data)); if isempty(t1)&&~isempty(deblank(t1)), error('incorrect syntax in onset field %s (condition %s, subject %d, session %d)',onsets{data}, name, nsub, nses); end
                    ...
                    t1=cellfun(@str2num,durations(data)); if isempty(t1)&&~isempty(deblank(t1)), error('incorrect syntax in duration field %s (condition %s, subject %d, session %d)',onsets{data}, name, nsub, nses); end
