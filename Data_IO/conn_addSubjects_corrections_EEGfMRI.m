...
% analysis:
function [sub_dir, sub_dir_mod] = conn_addSubjects_corrections_EEGfMRI(study_name,modality,base_path_data, scan_parameters,study_conditions, chanlocs_file, replace_files)
...
for i = 1:length(subject_folder_vect)
    ...
    ORIG_MRI_start_BLOCKIDX_vect = cell(1,size(task_blocks_dir,1));
    ORIG_MRI_end_BLOCKIDX_vect = cell(1,size(task_blocks_dir,1));
    ...
    EEG_vhdr = cell(1,size(task_blocks_dir,1));
    EEG_mat = cell(1,size(task_blocks_dir,1));
    saved_final_data = cell(1,size(task_blocks_dir,1));

    
    for j = 1:size(task_blocks_dir,1)
        
        fprintf(['\n ***************************** Processing ParticipantID ' curr_ID ' TaskBlock ' num2str(j) ' Run ' 'task' ' ***************************** \n']);
        ...
        output_dir = [subject_dir filesep task_blocks_dir(j).name]; 
        output_filename_vhdr = ['task_' curr_ID '_VHDR_TaskBlock' num2str(j)]; vhdr_dir = dir([output_dir filesep output_filename_vhdr '.set']);
        output_filename_mat = ['task_' curr_ID '_MAT_TaskBlock' num2str(j)];  mat_dir = dir([output_dir filesep output_filename_mat '.set']);

        %%%%%%%%%%%%% Correct the MRI_start_BLOCKIDX and MRI_end_BLOCKIDX based on the difference in slice markers for the data
        %%%%%%%%%%%%% saved in VHDR format and the data stored in the MAT file
        
        if isempty(vhdr_dir) || isempty(mat_dir) || replace_files
            % Load data from the backup .vhdr datafile:
            curr_file = [subject_dir filesep 'Task_block_' num2str(j) filesep curr_ID '_full_dataset_' num2str(j) '.vhdr']; skip_analysis = isempty(dir(curr_file));
            if ~skip_analysis
                [curr_filepath, curr_filename] = fileparts(curr_file);
                EEG_vhdr{j} = cust_loadbv(curr_filepath,[curr_filename '.vhdr']);
                EEG_vhdr{j} = pop_chanedit(EEG_vhdr{j},'load',{chanlocs_file 'filetype' 'autodetect'});
                EEG_vhdr{j}.setname = ['task_' curr_ID '_VHDR_TaskBlock' num2str(j)]; EEG_vhdr{j} = eeg_checkset(EEG_vhdr{j});
                
                % Correct the number of slice markers if there are extra from Pre-scanning procedures:
                [EEG_vhdr{j}] = remove_extra_slicemarkers_EEGfMRI(EEG_vhdr{j},scan_parameters, scan_parameters.tfunc_num_images);
            end
            
            % Load data from the saved .mat datafiles:
            curr_block_dir = [subject_dir filesep task_blocks_dir(j).name];
            final_data = curate_matfiles_EEGfMRI(curr_block_dir,study_base_name,scan_parameters); final_EEG = final_data.final_EEG;
            [EEG_mat{j},final_data] = curate_mat2set_EEGfMRI(final_data,chanlocs_file,['task_' curr_ID],scan_parameters,EEG_vhdr{j}.event);
            EEG_mat{j}.setname = [EEG_mat{j}.setname '_TaskBlock' num2str(j)];
            % saved_final_data{j} = final_data;
            
            % Add condition markers to both EEG streams:
            % Update the condition details based on the aligned slice markers:
            if isfield(final_data,'removed_start_latencies')
                curr_tD_file = [];
                curr_tD_file.MRI_start_BLOCKIDX_vect{j} = MRI_start_BLOCKIDX_vect{j};
                curr_tD_file.MRI_end_BLOCKIDX_vect{j} = MRI_end_BLOCKIDX_vect{j};
                
                curr_tD_file = EEGfMRI_update_aligned_volumes(curr_tD_file,final_data,scan_parameters,1,j);
                
                ORIG_MRI_start_BLOCKIDX_vect{j} = curr_tD_file.ORIG_MRI_start_BLOCKIDX_vect{j};
                ORIG_MRI_end_BLOCKIDX_vect{j} = curr_tD_file.ORIG_MRI_end_BLOCKIDX_vect{j};
                MRI_start_BLOCKIDX_vect{j} = curr_tD_file.MRI_start_BLOCKIDX_vect{j};
                MRI_end_BLOCKIDX_vect{j} = curr_tD_file.MRI_end_BLOCKIDX_vect{j};
            end
        ...
        %%%%%%%%%%%%% Save Compiled EEG for the trials in the current block %%%%%%%%%%%%%
        if (saveEEG  && (isempty(vhdr_dir) || isempty(mat_dir))) || (saveEEG && replace_files)       
            output_dir = [subject_dir filesep task_blocks_dir(j).name];
            pop_saveset(EEG_vhdr{j}, 'filename',EEG_vhdr{j}.setname,'filepath',output_dir);
            pop_saveset(EEG_mat{j}, 'filename',EEG_mat{j}.setname,'filepath',output_dir);
        end
        
        fprintf(['\n ***************************** Finished ParticipantID ' curr_ID ' TaskBlock ' num2str(j) ' Run ' 'task' ' ***************************** \n']);
    end
    
    clear EEG_mat EEG_vhdr
    if replace_files save([subject_dir filesep curr_ID '_trial_data']); end
