...
function [] = conn_precuratefMRI_PRT(subject_folder,scan_parameters,replace_files)
...
series_INFO = cell(size(unique_ser)-1); series_INFO_serNum = zeros(1,(length(unique_ser)-1)); 
for i = 1:(length(unique_ser)-1) 
    ...
    temp_serNum = strsplit(C{unique_ser(i)+1},':'); temp_serNum = cell2mat(temp_serNum(~isempty(temp_serNum)));
    series_INFO_serNum(i) = str2num(temp_serNum);
end    

%% Identify the anatomical, resting fMRI and task fMRI Series:

% This should get the series number of the following scans:
curr_anat = series_INFO_serNum(find(cell2mat(cellfun(@(x)~isempty(strfind(x{1},'FSPGR'))&(x{2} == anat_num_images),series_INFO,'UniformOutput',false)))); if length(curr_anat)>1 curr_anat = curr_anat(end); end
% curr_rsfunc = find(cell2mat(cellfun(@(x)~isempty(strfind(x{1},'RESTING'))&(x{2} == rsfunc_num_images),series_INFO,'UniformOutput',false)));
curr_tfunc = series_INFO_serNum(find(cell2mat(cellfun(@(x)~isempty(strfind(x{1},'FMRI'))&(x{2} == tfunc_num_images),series_INFO,'UniformOutput',false)))); 
...
cd(dcm2nii_path);
if ~exist_anat
    fprintf(['\n ***************************** Processing Anatomy MRI - Exam ' num2str(curr_exam) ' Series ' num2str(curr_anat) ' ***************************** \n']);
...
if ~exist_tfunc
    for i = 1:length(curr_tfunc)
        fprintf(['\n ***************************** Processing Task fMRI ' num2str(i) ' - Exam ' num2str(curr_exam) ' Series ' num2str(curr_tfunc(i)) ' ***************************** \n']);
