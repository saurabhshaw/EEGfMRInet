...
function output_data = conn_loadICA_data(CONN_cfg,scan_parameters)

% CONNresults_folder,CONN_data_type,CONN_project_name,net_to_analyze,use_All_cond

% Load CONN data:
CONNdata_timeseries = load([CONN_cfg.CONNresults_folder filesep CONN_cfg.CONN_data_type '.Timeseries.mat']); load([CONN_cfg.CONNresults_folder filesep CONN_cfg.CONN_project_name '.mat']);
fid = fopen([CONN_cfg.CONNresults_folder filesep CONN_cfg.CONN_data_type '.ROIs.txt']); CONN_node_labels = textscan(fid,'%s'); CONN_node_labels = CONN_node_labels{1}; fclose(fid); % These are the labels of the CONN ICA Components
...
CONN_cfg.p_norm = 2; % can switch from L1 to L2 norm
