...
% analysis:
function [sub_dir, sub_dir_mod, varargout] = conn_addSubjects_EEGfMRI_PRT(study_name,modality,base_path_data, scan_parameters,study_conditions, replace_files)
...
sub_block_condition_onset_vect = cell(1,length(subject_folder_vect));
sub_block_condition_duration_vect = cell(1,length(subject_folder_vect));
for i = 1:length(subject_folder_vect)
    ...
    for j = 1:size(task_blocks_dir,1)
        ...
        %%%%%%%%%%%%% Look for BrainStorm PRT files, in the absence of which, use custom blocks %%%%%%%%%%%%%
        curr_block_dir_PRT = dir([subject_dir filesep task_blocks_dir(j).name filesep '*.prt']);
        
        if ~isempty(curr_block_dir_PRT) %%%%%%%%%%%% PRT file found %%%%%%%%%%%%
            % Read the PRT file
            fid = fopen([curr_block_dir_PRT(1).folder filesep curr_block_dir_PRT(1).name]);
            file_data = []; ii = 1;
            while 1
                tline = fgetl(fid);
            ...
            % Find the text blocks related to each condition:
            file_condition_idx = zeros(1,length(study_conditions));
            for ii = 1:length(study_conditions)
                file_condition_idx(ii) = find(cellfun(@(x) ~isempty(strfind(x,study_conditions(ii))),file_data));
            end
            file_condition_trials = cellfun(@(x) str2num(x),[file_data(file_condition_idx + 1)]); % Find the number of trials for each condition (on the line after condition names in .PRT files)
            
            % Populate block_condition_onset_vect and block_condition_duration_vect:
            ...
            for k = 1:length(study_conditions)
                ...
                curr_condition_vect = file_data((file_condition_idx(k)+2) : (file_condition_idx(k)+2+file_condition_trials(k)-1));
                curr_condition_vect_split = cellfun(@(x)strsplit(x,' '),curr_condition_vect,'un',0);
                
                block_condition_onset_vect{j}{k} = cellfun(@(x) str2num(x{1}),curr_condition_vect_split)*TR; % Multiply by TR to convert to seconds
                block_condition_duration_vect{j}{k} = cellfun(@(x) str2num(x{2}) - str2num(x{1}),curr_condition_vect_split)*TR; % Multiply by TR to convert to seconds
                
            end
            
        else %%%%%%%%%%%% No PRT file found %%%%%%%%%%%%
    ...
    save_dir = dir([subject_dir filesep curr_ID '_trial_data.mat']);
    if isempty(save_dir) || replace_files
        save([subject_dir filesep curr_ID '_trial_data']);
    end
    
    sub_block_condition_onset_vect{i} = block_condition_onset_vect;
    sub_block_condition_duration_vect{i} = block_condition_duration_vect;
    
    %% Check if MRI files already processed by checking anatomical directory:
    if (~exist([subject_dir, filesep, 'MRI', filesep, 'anat' filesep num2str(curr_exam) '_anat.nii.gz']))
        fprintf(['\n ***************************** Starting MRI file processing - Subject P' curr_ID ' ***************************** \n']);
        if ~isempty(curr_block_dir_PRT)
            conn_precuratefMRI_PRT(subject_dir,scan_parameters,replace_files)
        else
            % conn_precuratefMRI(subject_folder,scan_parameters,replace_files)
            conn_precuratefMRI(subject_dir,scan_parameters,replace_files)
        end
    end
end

varargout{1} = sub_block_condition_onset_vect;
varargout{2} = sub_block_condition_duration_vect;
