...
function [EEG, com] = cust_loadbv(path, hdrfile, srange, chans)
...
% Header file
disp('cust_loadbv(): reading header file');
...
% Update the header information if the file has been renamed:
[~,file_name,~] = fileparts(hdrfile);
other_files = dir(strcat(path,filesep,file_name,'*'));
if sum(strcmp({other_files.name},[file_name '.eeg']))
    hdr.commoninfos.datafile = [file_name '.eeg'];
end
if sum(strcmp({other_files.name},[file_name '.vmrk']))
    hdr.commoninfos.markerfile = [file_name '.vmrk'];
    marker_rename = true;
end

% Open data file and find the number of data points
% -------------------------------------------------
disp('cust_loadbv(): reading EEG data');
...
if exist('chanlocs', 'var') && isfield(chanlocs, 'scale')
    disp('cust_loadbv(): scaling EEG data');
...
if isfield(hdr.commoninfos, 'markerfile')
    disp('cust_loadbv(): reading marker file');
    MRK = readbvconf(path, hdr.commoninfos.markerfile);
    
    % Update the MRK header information if the file has been renamed:
    if marker_rename
        MRK.commoninfos.datafile = hdr.commoninfos.datafile;
    end

    if hdr.commoninfos.datafile ~= MRK.commoninfos.datafile
        disp('cust_loadbv() warning: data files in header and marker files inconsistent.');
    end
...
if nargout == 2
    com = sprintf('EEG = cust_loadbv(''%s'', ''%s'', %s, %s);', path, hdrfile, mat2str(srange), mat2str(chans));
end
