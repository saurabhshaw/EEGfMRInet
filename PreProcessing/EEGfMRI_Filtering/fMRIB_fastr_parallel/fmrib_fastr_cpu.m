...
function EEG = fmrib_fastr_cpu(EEG,lpf,L,Window,Trigs,strig,anc_chk,tc_chk,Volumes,Slices,varargin)
warning('off','all')
...
% try
    parfor c = 1:m        % This is the parfor location
        % Progress bar Init
%         if c==1
%             barth=5;
%             barth_step=barth;
...
%         end
            ...
            for s=starts:lasts
                ...
                if strig==1 % Slice Triggers
                    ...
                    elseif ((s>(starts+hwinl+2)) & (s<=(lasts-(hwinl+2))))
                        curr_ss=s+hwinl; % changed from ss to curr_sss to make it compatible with par-for
                        switch art
                            case 1
                                slice_art1=[slice_art1(2:end,:);...
                                    Idata(secmarker(curr_ss)-pre_peak:...
                                    secmarker(curr_ss)+post_peak)];
                                avg_art=mean(slice_art1,1);
                                art=2;
                            case 2
 ...
 %%           
            ...
            if isempty(intersect(exc,c)) & npc~=0
                ...
                for apc_idx=2:pcs % changing index variable from apc to apc_idx - to make it compatible with par-for loops
                    papc(:,apc_idx)=papc(:,apc_idx)*minmax1/...
                        (max(papc(:,apc_idx))-min(papc(:,apc_idx)));
                end
