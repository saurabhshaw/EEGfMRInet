...
function EEG = fmrib_fastr_cpu_segment(EEG,lpf,L,Window,Trigs,strig,anc_chk,tc_chk,Volumes,Slices,varargin)
%%
...
if tc_chk==1
    if length(Trigs)~=(Slices*Volumes)
        if verbose fprintf('correcting slice triggers...\n'); end
...
if strig==1
    if Window>floor((ml-3)/2)
        ...
        hwinl=Window;
        if verbose warning('For %d slice triggers, the maximum averaging\nwindow length allowed is %d. Window length set to %d.\n',...
            ml,  Window,Window); end
    end
elseif strig==0
    if Window>(ml-2)
        Window=ml-2;
...
if verbose fprintf('Allocating memory...\n'); end
            ...
            for s=starts:lasts
                ...
                if s==1
                    try
                        ...
                        if c==1
                            if sec==1
                                if verbose warning...
                                    ('Not enough data to remove first artifact segment'); end
                                STARTFLAG=1;
                            end
                        end
                    end
                elseif sec==sections & s==lasts
                    try
                        ...
                        if c==1
                            if verbose warning('Not enough data to remove last artifact segment'); end
                            LASTFLAG=1;
                        end
                    end
                else
            ...
            if isempty(intersect(exc,c)) & npc~=0
                ...
                if sec==1
                    if ischar(npc)
                        ...
                        if verbose fprintf('\n%d residual PCs will be removed from channel %d\n . If you get an error "line 746 of fmrib_fastr: index exceeds matrix dimensions" it means there is an inconsistency in your TR triggers, either the TR length or the number of markers',pcs,c); end
                    else
                        pcs=npc;
                    end
                end
