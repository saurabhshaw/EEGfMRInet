...
function EEG = fmrib_fastr_gpu(EEG,lpf,L,Window,Trigs,strig,anc_chk,tc_chk,Volumes,Slices,varargin)
%%
...
ANCfwts=firls(filtorder,f,a_hp);
...
secmarker2 = gpuArray(secmarker);

% Construct Artifacts and Subtract
% This is where the code is parallelized
% ---------------------------------

% Formatting variable to work in a parfor loop:
EEGData = gpuArray(EEG.data);
% secmarker2 = num2cell(secmarker2);
%try
    for c = 1:m        
        % Progress bar Init
%         if c==1
...
%         end
        ...
        if strig==1
            slice_art1 = gpuArray.zeros(hwinl+1,art_length);
            slice_art2 = gpuArray.zeros(hwinl+1,art_length);
        else
            slice_art1 = gpuArray.zeros(2*hwinl+1,art_length);
        end
        avg_art = gpuArray.zeros(1,art_length);
        Noise = gpuArray.zeros(1,n);
        pcamat = gpuArray.zeros(floor(max(markerl)/2),pre_peak+max_postpeak+1);
        ...
        for sec=1:sections
            ...
            if L > 1
                if sec==1 & sections > 1
                    curr_t = (d1:(d1-1)+secl(sec)+pad_sec)*(1/fs);
                    upsample_t = curr_t(1):(1/(fs*L)):curr_t(end);
                    Idata = interp1(curr_t,tmpdata(d1:(d1-1)+secl(sec)+pad_sec),upsample_t,'linear');
                    Iorig = interp1(curr_t,EEGData(c,d1:(d1-1)+secl(sec)+pad_sec),upsample_t,'linear');
                    % Idata = interp(tmpdata(d1:(d1-1)+secl(sec)+pad_sec),L,4,1);
                    % Iorig = interp(EEGData(c,d1:(d1-1)+secl(sec)+pad_sec),L,4,1);
                elseif sec==1 & sections==1
                    curr_t = (d1:d2)*(1/fs);
                    upsample_t = curr_t(1):(1/(fs*L)):curr_t(end);
                    Idata = interp1(curr_t,tmpdata(d1:d2),upsample_t,'linear');
                    Iorig = interp1(curr_t,EEGData(c,d1:d2),upsample_t,'linear');
                    % Idata = interp(tmpdata(d1:d2),L,4,1);
                    % Iorig = interp(EEGData(c,d1:d2),L,4,1);
                elseif sec==sections
                    curr_t = ((d1-1)+...
                        sum(secl(1:sec-1))+1-pad_sec:d2)*(1/fs);
                    upsample_t = curr_t(1):(1/(fs*L)):curr_t(end);
                    Idata = interp1(curr_t,tmpdata((d1-1)+...
                        sum(secl(1:sec-1))+1-pad_sec:d2),upsample_t,'linear');
                    Iorig = interp1(curr_t,EEGData(c,(d1-1)+...
                        sum(secl(1:sec-1))+1-pad_sec:d2),upsample_t,'linear');
                    ...
                    curr_t = ((d1-1)+sum(secl(1:sec-1))+...
                        1-pad_sec:(d1-1)+sum(secl(1:sec))+pad_sec)*(1/fs);
                    upsample_t = curr_t(1):(1/(fs*L)):curr_t(end);
                    Idata = interp1(curr_t,tmpdata((d1-1)+sum(secl(1:sec-1))+...
                        1-pad_sec:(d1-1)+sum(secl(1:sec))+pad_sec),upsample_t,'linear');
                    Iorig = interp1(curr_t,EEGData(c,(d1-1)+sum(secl(1:sec-1))+...
                        1-pad_sec:(d1-1)+sum(secl(1:sec))+pad_sec),upsample_t,'linear');
            ...
            INoise = gpuArray.zeros(1,length(Idata));
                ...
                if s==1
                    try
                        if c==1
                            ...
                            C = gpuArray.zeros(1,length(B_idx));
 ...
 %%           
            ...
            %----------PCA of residuals-------------------
            fitted_res=gpuArray.zeros(length(INoise),1);
            
            if isempty(intersect(exc,c)) & npc~=0
                % Ipca=filtfilt(hpfwts,1,double(Idata-INoise));
                Ipca = filter(hpfwts,1,(Idata-INoise));
                pccount=1;
                skcount=1;
                ...
                if sec==1
                    if ischar(npc)
                        d_oev=find(double(abs(diff(oev)))<TH_SLOPE);
                        dd_oev=diff(d_oev);
                        for I=1:length(dd_oev)-3
                            if gather([dd_oev(I) dd_oev(I+1) dd_oev(I+2)]==[1 1 1])
                                break
                            end
                        end
                        SLOPETH_PC=d_oev(I)-1;
                        TMPTH=find(double(cumsum(oev))>TH_CUMVAR);
                        CUMVARTH_PC=TMPTH(1);
                        TMPTH=find(double(oev)<TH_VAREXP);
            ...
            if L > 1
                fcleanEEG=decimate2_gpu(Idata,L);
                fNoise=decimate2_gpu(INoise+fitted_res',L);
            else
                fcleanEEG=Idata;
...
%            end
        ...
        if lpf>0
            cleanEEG=filter(lpfwts,1,cleanEEG);
            Noise=filter(lpfwts,1,Noise);
        end
        ...
        if (anc_chk==1) && isempty(intersect(exc,c))
            ...
            % ---------------------------
            refs=Noise(d1:d2)';
            tmpd=filter(ANCfwts,1,cleanEEG)';
            d=double(tmpd(d1:d2));
