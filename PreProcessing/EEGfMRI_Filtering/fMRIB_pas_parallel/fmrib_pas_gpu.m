...
function EEG = fmrib_pas_gpu(EEG,QRSevents,method,npc)
...
%     % Defining the parallel variables:
%     fitted_art = cell(1,channels);

    % for the first window/2 points use arthemitic mean for averageing.
    % findg mean QRS peak-to-peak (R-to-R) interval
    parfor ch=1:channels-2
        
        % Defining variables:
        peakcount = length(peakI);
        pa = peakcount;
        curr_fitted_art = zeros(1,samples);
        ...
        % Check if the channel data doesn't have any inf values:
        if all(isfinite(double(EEG.data(ch,:))));
            ...
            try
                ...
                curr_fitted_art(peakI(1)-PArange:peakI(1)+PArange)=...
                    pad_fit';
            catch
            end
            ...
            %---------- Processing of the first part of the data ------------
            for p=2:GHW+1
                ...
                curr_fitted_art(peakI(p)-PArange:peakI(p)+PArange)=...
                    pad_fit';
            ...
            for p=GHW+2:peakcount-GHW-1
                ...
                curr_fitted_art(peakI(p)-PreP:peakI(p)+PostP)=...
            ...
            fitted_art(ch,:) = curr_fitted_art;
            
            disp(strcat('Finished processing channel number ',num2str(ch)));
        end
