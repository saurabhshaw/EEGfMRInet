function EEG = offline_preprocess_auto(cfg,curr_dir,dataset_name,overwrite_files,EEG)
...
num_steps = 8; % Number of steps in the processing pipeline
run_second_ICA = 0;
...
if overwrite_files || isempty(dir(stageCompletion_file))
...
%% 2 - Segment data according to data type
current_stage = 2;
if max_finishedStage == current_stage-1
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Stage Code %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if segment_data
...
if max_finishedStage == current_stage-1
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Stage Code %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    num_ICA_reruns = 2;
    EEG_ICA = cell(1,num_ICA_reruns); EEG_ICA_IClabelrejcomp = cell(1,num_ICA_reruns); EEG_ICA_Manualrejcomp = cell(1,num_ICA_reruns);
    ...
    EEG_ICA_IClabelrejcomp{1} = components;
...
%% 5 - ReRun ICA and reject components that are not Brain or Other (according to ICLabel):
current_stage = 5;
if max_finishedStage == current_stage-1
    ...
    if ~isempty(prevStage_iscomplete) % Check if previous stage is complete and then proceed with rest of the analysis
        ...
        components = find(EEG_ICLabel_max_bin_final);
        EEG_ICA_IClabelrejcomp{2} = components;
        EEG = pop_subcomp(EEG, components, 0);
        EEG = pop_iclabel(EEG,'default'); EEG = eeg_checkset( EEG );
...
%% 8 - Interpolate bad channels identified earlier:
current_stage = 8;
if max_finishedStage == current_stage-1
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Stage Code %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if max_finishedStage == current_stage
    ...
    save([curr_dir_preprocessed filesep dataset_name '_preprocessed-Workspace']);
