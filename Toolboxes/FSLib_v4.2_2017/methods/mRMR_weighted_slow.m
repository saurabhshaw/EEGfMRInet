function [fea, score] = mRMR_weighted_slow(X_train, Y_train, weights, K)
...
t1=cputime;
for i=1:nd
   t(i) = WeightedMIToolbox('mi', weights, X_train(:,i),Y_train);
...
for k=2:K
    ...
    for i=1:ncand
        t_mi(i) = WeightedMIToolbox('mi', weights ,X_train(:,idxleft(i)),Y_train);
...
for i=1:size(da,2)
    c(i) = WeightedMIToolbox('mi', weights, da(:,i), dt);
