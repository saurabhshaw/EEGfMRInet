function signal = clean_flatlines_mod(signal,max_flatline_duration,max_allowed_jitter)
...
for c=1:signal.nbchan
    signalCurr = signal.data(c,:);
    signalCurr = (signalCurr - mean(signalCurr))./std(signalCurr);
    zero_intervals = reshape(find(diff([false abs(diff(signalCurr))<(max_allowed_jitter*eps) false])),2,[])';
