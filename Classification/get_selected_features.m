function [final_Feature] = get_selected_features(curr_model, Featurefiles_basename, Featurefiles_directory, run_matfile, varargin)
...
if ~isempty(varargin) Feature_idx = varargin{1}; else Feature_idx = 1:length(currFeatures_curated); end
...
for i = 1:length(currFeatures_curated)
    ...
    Feature_labels_IDX = cellfun(@(x) str2num(x{1}),all_Feature_labels) == Feature_idx(i);
