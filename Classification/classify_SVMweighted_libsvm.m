function [TrainAccuracy, TestAccuracy, Model] = classify_SVMweighted_libsvm(X,Y,kernel,testTrainPartition,class_weights,varargin)
...
train_control_string = '-s 0'; % C-SVC
%train_control_string = '-s 1'; % nu-SVC
...
for i = 1:length(class_weights)
    train_control_string = [train_control_string, ' -w' num2str(i-1) ' ' num2str(floor(100*class_weights(i)))];
end
...
if optimize
    if optimize_kernel
        ...
        [bestc, bestg, bestker] = crossvalopt_SVM_libsvm(X, Y, optimize_kernel);
        ...
        [bestc, bestg] = crossvalopt_SVM_libsvm(X, Y, optimize_kernel);
