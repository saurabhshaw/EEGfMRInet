function [output_features, output_scores,dataset_mRMR,feature_labels_mRMR] = mRMR_weighted_iterate_loop(curr_dataset_mRMR,curr_labels_mRMR,weights,curr_feature_size,max_features,dataset_mRMR,feature_labels_mRMR,varargin)
% These are the lines that are run at each level of the loop to get the multi-level mRMR output:

%% Run mRMR:
[output_features, output_scores] = mRMR_weighted(curr_dataset_mRMR,curr_labels_mRMR,weights,min(max_features,size(curr_dataset_mRMR,2)));
